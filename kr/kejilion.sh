...
LIMITS
	...
	echo -e "${gl_lv}内存: ${MEM_MB}MB | 혼잡 알고리즘:${CC} | 队列: ${QDISC}${gl_bai}"
}
...
Kernel_optimize() {
	...
	while true; do
	  ...
	  local current_mode=$(grep "^# 模式:" /etc/sysctl.d/99-kejilion-optimize.conf 2>/dev/null | sed 's/# 모드: //' | awk -F'|' '{print $1}' | xargs)
...
add_connection() {
	...
	kj_ssh_read_host_user_port "请输入IP地址: " "请输入用户名 (默认: root): " "포트 번호를 입력하십시오(기본값: 22):" "root" "22"
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}-------------------------"
	  echo -e "${gl_kjlan}71.  ${color71}Navidrome私有音乐服务器             ${gl_kjlan}72.  ${color72}비트워든 비밀번호 관리자${gl_huang}★${gl_bai}"
...
kpanel_system_resource_hosts_failure() {
	...
	if kpanel_system_resource_restore_hosts "$path" "$backup" "$original_version"; then
		...
		if recovery_path="$(kpanel_system_resource_persist_recovery_snapshot "$snapshot_dir" hosts)"; then
			...
			kpanel_system_resource_error "호스트 실패 스냅샷 지속성 실패, 호스트 표시 백업 경로가 생성되지 않음"
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}3.   ${gl_bai}사용자 비밀번호 로그인 모드${gl_kjlan}4.   ${gl_bai}安装Python指定版本"
	  ...
	  echo -e "${gl_kjlan}23.  ${gl_bai}전류 제한 자동 종료${gl_kjlan}24.  ${gl_bai}用户密钥登录模式"
